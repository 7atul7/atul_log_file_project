...
class Session:
    ...
    raw = spark.read.text("/content/drive/MyDrive/updated_log_ip_request.txt")
    ...
    def data_from_s3_bucket(self):
        try:
            self.raw = self.spark.read.text("/content/drive/MyDrive/updated_log_ip_request.txt")
...
class RawLayer(Session):
    ...
    def drop_duplicate_records(self):
        self.raw=self.raw.drop_duplicates(["client/ip","datetime","method"])\
        .drop("row_id")
        self.raw=self.raw.withColumn("row_id", monotonically_increasing_id())
        self.raw=self.raw.select("row_id","client/ip","datetime","method","request","status_code","size","referer","user_agent")
    ...
    def save_to_s3(self):
        self.raw.coalesce(1).write.csv("/content/drive/MyDrive/project_output_files/raw_log_details/", mode="overwrite",header=True)

    def save_to_hive_table(self):
        pass
        # self.raw.coalesce(1).write.option("mode","overwrite").saveAsTable('Raw_Data9')
        self.spark.sql("select count(*) from Raw_Data9").show()
